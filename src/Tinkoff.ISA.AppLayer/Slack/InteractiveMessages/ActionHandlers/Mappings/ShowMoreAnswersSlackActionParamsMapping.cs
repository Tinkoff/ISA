...
{
    public class ShowMoreAnswersSlackActionParamsMapping : Profile
    {
        public ShowMoreAnswersSlackActionParamsMapping()
        {
            CreateMap<InteractiveMessage, ShowMoreAnswersSlackActionParams>()
                .ForMember(d => d.AttachmentId, opt => opt.MapFrom(src => src.AttachmentId - 1))
                .ForMember(d => d.ButtonParams,
                    opt => opt.MapFrom(src =>
                        JsonConvert.DeserializeObject<ShowMoreAnswersActionButtonParams>(src.Actions.First().Value)));
        }
    }
}
