...
{
    public class HelpedSlackActionParamsMappingTests
    {
        private readonly IMapper _mapper;

        public HelpedSlackActionParamsMappingTests()
        {
            var config = new MapperConfiguration(cfg => cfg.AddProfile<HelpedSlackActionParamsMapping>());
            _mapper = new Mapper(config);
        }

        [Fact]
        public void Map__InteractiveMessage_To_HelpedSlackActionParams__AllNecessaryFieldsAreMapped()
        {
            // Arrange
            ...
            {
                Value = JsonConvert.SerializeObject(new HelpedAnswerActionButtonParams{QuestionId = questionId, AnswerId = answerId})
            };
            ...
            // Act
            var destination = _mapper.Map<InteractiveMessage, HelpedSlackActionParams>(source);
